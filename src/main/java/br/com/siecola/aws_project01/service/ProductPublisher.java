...
public class ProductPublisher {
    ...
    public void publishProductEvent(Product product, EventType eventType, String username){
        ...
        try {
            envelope.setData(objectMapper.writeValueAsString(productEvent));//converte para json

            //publica
            snsClient.publish(
                    productEventsTopic.getTopicArn(),
                    objectMapper.writeValueAsString(envelope)//converte para json
            );

        } catch (JsonProcessingException e) {
            LOG.error("Falha ao criar evento do produto");
        }
